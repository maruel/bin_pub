  ...
  for dirpath, dirnames, filenames in os.walk(path):
    for index in xrange(len(dirnames) - 1, -1, -1):
      ...
      if blacklisted(converted_d):
        # This speeds up searching by blacklisting directories that should not
        # be looked into like .git or .svn.
        dirnames.pop(index)
...
def obj_dump(obj, **kwargs):
  ...
  if isinstance(obj, dict):
    kwargs['maxlen'] -= 2
    if recurse:
      keys = sorted(k for k in obj.iterkeys()
                    if not isinstance(k, basestring) or not k.startswith('__'))
      inner = clamp_str(
          ', '.join(
              '%s=%s' % (obj_dump(k, **kwargs), obj_dump(obj[k], **kwargs))
              for k in keys),
          **kwargs)
    else:
      inner = 'len=%d' % len(obj)
  ...
  if isinstance(obj, (frozenset, set)):
    if recurse:
      inner = clamp_str(', '.join(obj_dump(o, **kwargs) for o in obj), **kwargs)
    else:
      inner = 'len=%d' % len(obj)
...
def log_function(f, name=None, logcall=True, logreturn=True, outfn=None,
    **outer_kwargs):
  """Logs function calls.
